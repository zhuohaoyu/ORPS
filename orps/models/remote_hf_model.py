    ...
    async def query(
        ...
        while num_retries > 0:
            ...
            try:
                ...
                self.logger.exception("Failed to query remote_hf API")
            finally:
                await self.client_pool.release_client(client)
            if response is not None:
                break
            else:
                self.logger.error(f"Empty response from remote_hf inference API, retries left: {num_retries}")
...
def run_remote_hf_inference(
    ...
    request_limit: Optional[int] = 10000,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--num-retries", type=int, default=1)
