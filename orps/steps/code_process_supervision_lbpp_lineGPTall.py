...
CRITIC_SYSTEM_PROMPT = f"""You are a competitive programming analyst. You will provide feedback and a score based on the contestant's thought about the problem. You only need to give a score based on the latest line. Your output can only be a number from 1, 2, 3, 4, 5, and it should be wrapped in $$, for example: $$3$$. Your score should be based on the completeness of the thought; the more complete the thought, the higher the score. Additionally, you need to consider the correctness of the thought; the more correct the thought, the higher the score. If you think the thought is already end, you should generate <|thought_end|> too, and the score should be 5."""
...
class CodeProcessSupervisionLbpp_lineGPTall(BaseStep):  
    ...
    def init_programmer_model(self, type: str, inference_kwargs: Dict, prompt_postprocessor_config: Dict = None, **kwargs):
        ...
        assert type == "openai", "Only openai is supported for programmer model"
