...
PROGRAMMER_SYSTEM_PROMPT = """You are a Python programmer in a code interview. You're solving a coding problem while sharing your thoughts and discussing with an critic. Always follow the format below for each round:
...
CRITIC_SYSTEM_PROMPT = f"""You are a technical critic reviewing a programmer's Python code. Provide clear, constructive feedback and suggest improvements or propose a new approach. The programmer's thoughts, code, execution analysis will be provided to you each round and you need to give constructive feedback to the programmer.
...
2. Give a score for the programmer's code, thoughts to rate the overall performance so far
    - Scoring Guide: 5: The code is correct, efficient and discussion is smooth, the reasoning and thoughts are clear. 4: The code is correct, but the thoughts are slightly flawed, or the code is not efficient. 3: The code has some correctness issues, or the plan is not good. 2: The code is incorrect, or the reasoning is not clear. 1: The code is incorrect, or the reasoning is not clear.
    - Be objective and focus on the code and the thoughts, the thought process of the programmer is very important
    - If you would like to give score 5, and believe you should give even higher than 5, you could terminate the interview by giving an additional token: <|good|> at the end of Thoughts section.

3. {metrics_description}

4. Always format your response as follows, this is very important:
# === BEGIN CRITIC THOUGHTS ===
# [Your thoughts according to the above rules]
# === END CRITIC THOUGHTS ===
# === BEGIN SCORE ===
$$only the integer score between 1 to 5, nothing else, surrounded by LaTeX characters (two dollar signs on both sides)$$
# === END SCORE ===
Output strictly as shown above. Below are the problem-specific information in our interview.
""" + """\n{base_prompt}"""

PROGRAMMER_USER_PROMPT_FIRST_ROUND = """Here's your coding task:
{base_prompt}

Think carefully and write your solution strictly following our format and rules."""
...
Please improve your solution based on our discussion and analysis. Remember to strictly follow our format and rules."""

CRITIC_USER_PROMPT = """Let's review the reasoning and the solution to the problem:
...
Think carefully, write down your inner thoughts and give your feedback strictly following our format and rules."""
...
class CodeProcessSupervisionLbpp(BaseStep):  
    ...
    def generate_origin_node_from_lbpp(self):
        ...
        for inst in self.origin_instruction_dataset.instructions:
            if not inst.extra['analysis'].get('analysis_failed', False):
                ...
                parse_simple_analysis = get_analysis_string_from_dict(simple_analysis)
                ...
                parse_simple_analysis = "Analysis failed! Sorry for that. I cannot provide any analysis for this code." + get_analysis_string_from_dict(failed_analysis)
            ...
            msgs = [
                {"role": "system", "content": CRITIC_SYSTEM_PROMPT.format(base_prompt=base_prompt)}
            ]

            msgs.append({"role": "user", "content": CRITIC_USER_PROMPT.format(code=code, analysis=parse_simple_analysis)})
            ...
            if self.prompt_postprocessor:
                critic_inst.prompt = self.prompt_postprocessor.get_full_prompt_from_conversation(msgs)
            else:
        ...
        inference_kwargs = {**self.inference_kwargs, "output_path": current_critic_output_path}
        ...
        try:
            self.inference_function(self.origin_critic_instruction_dataset, **inference_kwargs)
        except Exception as e:
    ...
    def single_round_search(self, round_idx: int):
        ...
        for node in tqdm(last_node_list, desc=f"Generating programmer instructions of round {round_idx}"):
            ...
            for programmer_output, analysis, critique in zip(node.history_programmer_outputs, node.history_analyses, node.history_criticisms):
                parse_analysis = get_analysis_string_from_dict(CodeNode.parse_analysis_str(analysis))
        ...
        for inst in tqdm(current_programmer_instruction_dataset.instructions, desc=f"Generating critique instructions of round {round_idx}"):
            ...
            if not inst.extra['analysis'].get('analysis_failed', False):
                ...
                simple_analysis_str = json.dumps(failed_analysis, ensure_ascii=False)
                simple_analysis_str = f"Analysis failed! Sorry for that. I cannot provide any analysis for this code.\n{simple_analysis_str}"
                parse_simple_analysis = "Analysis failed! Sorry for that. I cannot provide any analysis for this code.\n" + get_analysis_string_from_dict(failed_analysis)
                score = 0.0
            
            msgs.append({"role": "user", "content": CRITIC_USER_PROMPT.format(
                code=programmer_output,
                analysis=parse_simple_analysis
            )})
        ...
        try:
            self.inference_function(current_critic_instruction_dataset, **inference_kwargs)
        except Exception as e:
    ...
    def postprocess(self, context: Context):
        ...
        for node in tqdm(all_best_nodes, desc="Analyzing best nodes with test data"):
            if self.dataset_type == "lbpp":
                try:
                    dynamic_analysis = dynamic_analyze_code_lbpp(node.current_code, node.task_id, self.lbpp_test_data, temp_dir, use_trace_back=self.if_use_trace_back)
                except Exception as e:
                    self.logger.error(f"Failed to analyze node {node.uuid} with test data. task_id: {node.task_id}, error: {e}")
                    dynamic_analysis = {}
            elif self.dataset_type == "lbpp_alter":
                try:
                    dynamic_analysis = dynamic_analyze_code_lbpp_alter(node.current_code, node.task_id, self.lbpp_test_data, temp_dir, use_trace_back=self.if_use_trace_back)
                except Exception as e:
                    self.logger.error(f"Failed to analyze node {node.uuid} with test data. task_id: {node.task_id}, error: {e}")
                    dynamic_analysis = {}
            else:
