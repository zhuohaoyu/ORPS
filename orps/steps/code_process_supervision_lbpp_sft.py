...
CRITIC_SYSTEM_PROMPT = """You are a helpful code reviewer, you will be guiding a programmer by sharing thoughts on the problem, solution, and then give a score for the overall reasoning process between you and the programmer, as well as the code quality. Your score should be a integer ranged between 1 to 5, where 5 stands for a very good reasoning path and a great solution. Now, this is the problem and test cases you will be discussing: {base_prompt}
"""
...
class CodeProcessSupervisionLbppSft(BaseStep):  
    ...
    def preprocess(self, context: Context):
        ...
        self.logger.info(f"len(self.lbpp_data): {len(self.lbpp_data)}")
        for task_id, _ in self.lbpp_data.items():
            self.logger.info(f"task_id: {task_id}")
        self.logger.info(f"len(self.lbpp_test_data): {len(self.lbpp_test_data)}")
        for task_id, _ in self.lbpp_test_data.items():
            self.logger.info(f"task_id: {task_id}")
