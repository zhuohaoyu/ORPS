...
from orps.lbpp_utils import CodeNode, SELECT_NODE_KEYS_ASCEND_SYMBOL
...
CRITIC_SYSTEM_PROMPT = f"""You are a technical interviewer reviewing a candidate's Python code. Provide clear, constructive feedback and suggest improvements or propose a new approach.
Your feedback should be strictly in the following format:

Review: [1-2 key observations + 1 suggestion]
Score: [1-10] (brief reason)

Keep feedback under 200 chars. Add <|good|> if solution is optimal. Note that you are the interviewer, not the candidate, but you are also thinking about how to obtain the optimal solution with the candidate. No chit-chat, no small talk.""" + """\n{base_prompt}"""

PROGRAMMER_USER_PROMPT_FIRST_ROUND = """Here's your coding task:
{base_prompt}

Think carefully and write your solution strictly following the format."""

PROGRAMMER_USER_PROMPT_LATER_ROUNDS = """Here's the execution analysis of the previous solution:

And my feedback as interviewer:
<critic>
{criticism}
</critic>

Please improve your solution based on this feedback. Remember to explain your thoughts in comments first."""

CRITIC_USER_PROMPT = """Let's review this solution:

Code:
{code}

Provide your feedback following the format above."""

class CodeReflexsionLbpp(BaseStep):  
    ...
    def __init__(
            ...
            data_group_tuple: tuple[int, int] = (1, 0),
            num_rounds: int = 5,
        ...
        super().__init__(step_type="code_reflexsion_lbpp", step_name=step_name, description="Code Reflexsion Lbpp")
    ...
    def preprocess(self, context: Context):
        self.logger.info("Preprocessing Code Reflexsion Lbpp") 
    ...
    def generate_origin_node_from_lbpp(self):
        ...
        try:
            ...
            max_input_tokens = 10000
            for _ in range(self.inference_max_retries):
                self.logger.info(f"Retrying {_+1}th critique instruction inference, max_input_tokens: {max_input_tokens}")
                for inst in self.origin_critic_instruction_dataset.instructions:
                    if self._count_tokens(inst.prompt) > max_input_tokens:
                        self.origin_critic_instruction_dataset.instructions.remove(inst)
                        for origin_inst in self.origin_instruction_dataset.instructions:
                            if origin_inst.uuid == inst.extra['origin_inst_uuid']:
                                self.origin_instruction_dataset.instructions.remove(origin_inst)
                                break
                try:
                    self.inference_function(self.origin_critic_instruction_dataset, **inference_kwargs)
                    break
    ...
    def single_round_search(self, round_idx: int):
        ...
        try:    
            ...
            max_input_tokens = 10000
            for _ in range(self.inference_max_retries):
                self.logger.info(f"Retrying {_+1}th programmer instruction inference, max_input_tokens: {max_input_tokens}")
                for inst in current_programmer_instruction_dataset.instructions:
                    if self._count_tokens(inst.prompt) > max_input_tokens:
                        for node in last_node_list:
                            if node.uuid == inst.extra['node_uuid']:
        ...
        for inst in tqdm(current_programmer_instruction_dataset.instructions, desc=f"Generating critique instructions of round {round_idx}"):
            for node in last_node_list:
                if node.uuid == inst.extra['derived_from_node_uuid']:
                    ...
                    break
            assert not derived_from_node.exploration_truncated and not derived_from_node.exploration_early_over, "The derived from node should not be truncated or early over"
            task_id = inst.extra['task_id']
            task_data = self.lbpp_data_group.get(task_id)
            if not task_data:
                ...
                for inst in current_critic_instruction_dataset.instructions:
                    if self._count_tokens(inst.prompt) > max_input_tokens:
                        for programmer_inst in current_programmer_instruction_dataset.instructions:
                            if programmer_inst.uuid == inst.extra['programmer_inst_uuid']:
                                break
                        for node in last_node_list:
                            if node.uuid == programmer_inst.extra['node_uuid']:
                                node.exploration_truncated = True
                                break
                        current_critic_instruction_dataset.instructions.remove(inst)
                        current_programmer_instruction_dataset.instructions.remove(programmer_inst)
                try:
                    self.inference_function(current_critic_instruction_dataset, **inference_kwargs)
                    break
                except Exception as e:
        ...
        for inst in current_programmer_instruction_dataset.instructions:
            ...
            if not inst.extra['analysis'].get('analysis_failed', False):
                ...
                assert simple_analysis['final_score'] == inst.extra['analysis']['score'], "The final score and the score in simple analysis should be the same"
                score = inst.extra['analysis']['score']
            else:
                code = inst.output
                failed_analysis = inst.extra['analysis']['quality_metrics']
            ...
            node = CodeNode(code, inst.output, simple_analysis_str, criticism, score, parent=parent_node, task_id=inst.extra['task_id'], current_process_score=0)
