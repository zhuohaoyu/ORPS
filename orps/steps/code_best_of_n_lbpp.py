...
class CodeBestOfNLbpp(BaseStep):  
    ...
    def __init__(
        ...
        super().__init__(step_type="code_best_of_n_lbpp", step_name=step_name, description="Code Best Of N Lbpp")
    ...
    def preprocess(self, context: Context):
        self.logger.info("Preprocessing Code Best Of N Lbpp") 
    ...
    def postprocess(self, context: Context):
        ...
        if self.scaling != 0:   
            for task_id, nodes in task_id_2_nodes.items():
                nodes[:] = nodes[:self.scaling]
                print(f"task_id: {task_id}, len(nodes): {len(nodes)}")
        ...
        for task_id, nodes in task_id_2_nodes.items():
            ...
            print(f'task_id: {task_id}, len(node_infoes_list1): {len(node_infoes_list)}')
            if self.scaling != 0:
                node_infoes_list = node_infoes_list[:self.scaling]
            ...
            print(f'task_id: {task_id}, len(node_infoes_list2): {len(node_infoes_list)}')
        ...
        if self.scaling != 0:
            save_best_nodes_path = os.path.join(save_path, f"best_nodes-{select_method_str}-{self.scaling}")
        else:
