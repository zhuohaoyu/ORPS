...
if __name__ == "__main__":
    ...
    handler.setLevel(logging.INFO)  # Set CLI handler to INFO
    ...
    log_filename = os.path.join(args.log, f'{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.log')

    
    file_handler = logging.FileHandler(log_filename)
    file_handler.setLevel(logging.DEBUG)  # Set file handler to DEBUG
    file_formatter = logging.Formatter(
        '%(asctime)s %(levelname)s %(name_last)-16s %(message)s', datefmt='%Y-%m-%d %H:%M:%S'
    )
    ...
    from orps.core.pipelines import EvaluationPipeline
